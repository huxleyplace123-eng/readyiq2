...
export function SiteNav({ route }: { route: Route }) {
  ...
  const who: [string, string][] = [["loan-officers/", "Loan officers — who’s not ready, progressing, ready to review"], ["platform/", "Mortgage companies — declines as a managed pipeline"], ["credit-repair/", "Credit repair firms — win and keep LO relationships"], ["consumers/", "Borrowers — your stage and your next step"], ["partners/", "Realtors and buildings — a link and a status"]];
  return <header className={`site-nav ${open ? "open" : ""}`}>
    <div className="site-nav-inner">
      <Brand />
      <nav className="site-links">
        <a href="platform/" className={is("platform") ? "on" : ""}>How it works</a>
                <div className={`dd ${dd === "w" ? "show" : ""}`} onMouseEnter={() => show("w")} onMouseLeave={hideSoon}><button className={is("loan-officers") || is("consumers") || is("partners") || is("credit-repair") ? "on" : ""} onClick={() => (dd === "w" ? setDd(null) : show("w"))}>Who it’s for ▾</button><div className="dd-menu"><div className="dd-box">{who.map(([h, l]) => <a key={h} href={h}>{l}</a>)}</div></div></div>
      ...
      <div className="site-actions"><a className="site-try" href={DEMO + "?mode=lender&lpage=borrowers"}>See the LO view</a><a className="site-book" href="book-a-demo/">Book a demo</a><a className="site-signin" href="sign-in/">Sign in</a><a className="site-demo" href={DEMO + "?mode=lender&lpage=start"}>Get your link <span>→</span></a><button className="site-burger" onClick={() => setOpen(!open)} aria-label="Menu">☰</button></div>
    </div>
    {open && <div className="site-mobile"><a href="platform/">Platform</a>{products.map(([h, l]) => <a key={h} href={h}>{l.split(" — ")[0]}</a>)}{who.map(([h, l]) => <a key={h} href={h}>{l.split(" — ")[0]}</a>)}<a href="integrations/">Integrations</a><a href="trust/">Trust</a><a href="resources/">Resources</a><a href={DEMO + "?mode=lender&lpage=start"}>Get started</a><a href="book-a-demo/">Book a demo</a><a href={DEMO + "?mode=partner"}>See the credit-repair view</a><a href="sign-in/">Sign in</a></div>}
  </header>;
}

export function HonestyStrip() {
  return <div className="honesty"><span>✓ Stages, never a claimed mortgage score</span><span>✓ Blockers from the report, not the number</span><span>✓ ReadyIQ never bills the borrower</span><span>✓ Flat pricing — never per referral</span></div>;
}

export function CtaBand({ title = <>Stop losing <em>not-ready borrowers.</em></>, sub = "Give every decline a stage, a plan, and a moment to come back. You’ll know who’s not ready, who’s progressing, and who’s ready to review." }: { title?: ReactNode; sub?: string }) {
...
export function SiteFooter() {
  ...
  return <footer className="site-footer">
    <div className="site-footer-inner">
      <div className="site-footer-brand"><Brand /><p>The bridge from not ready to ready to review. Not a credit repair company. Not a CRM. Never paid per referral.</p><small>Consumer credit tools — not a mortgage approval or lending decision. You can apply for a mortgage at any time; this is not required.</small></div>
      ...
      {col("Who it’s for", [["loan-officers/", "Loan officers"], ["credit-repair/", "Credit repair firms"], ["consumers/", "Borrowers"], ["partners/", "Partners"], ["resources/", "Resources & FAQ"], ["book-a-demo/", "Book a demo"], ["sign-in/", "Sign in"]])}
    </div>
...
/* ---------- page anatomy ---------- */
type Layout = "split" | "rev" | "center" | "narrow" | "band" | "grid" | "quote";
...
function Sections({ list }: { list: Section[] }) {
  ...
  return <>{list.map((s, i) => {
    ...
    if (layout === "quote") return <section key={i} className={`site-section lay-quote ${s.dark ? "dark" : ""}`}><div className="site-section-inner one"><span className={`section-kicker ${s.dark ? "light" : ""}`}>{s.kicker}</span><blockquote>{s.title}</blockquote><div className="site-body">{s.body}</div></div></section>;
...
const Steps = ({ items, row = false }: { items: [string, string, string?][]; row?: boolean }) => <ol className={`steps ${row ? "row" : ""}`}>{items.map(([t, d, m], i) => <li key={t}><span>{String(i + 1).padStart(2, "0")}</span><div><strong>{t}</strong><p>{d}</p>{m && <small>{m}</small>}</div></li>)}</ol>;
const Compare = ({ cols, rows, hi = 0 }: { cols: string[]; rows: string[][]; hi?: number }) => <div className="compare"><table><thead><tr><th /> {cols.map((c, i) => <th key={c} className={i === hi ? "hi" : ""}>{c}</th>)}</tr></thead><tbody>{rows.map((r) => <tr key={r[0]}><th>{r[0]}</th>{r.slice(1).map((v, i) => <td key={i} data-label={cols[i]} className={`${i === hi ? "hi" : ""} ${v.startsWith("✓") ? "yes" : v === "—" || v.startsWith("never") ? "no" : ""}`}>{v}</td>)}</tr>)}</tbody></table></div>;
const InOut = () => <div className="inout"><div><strong>Inside the Passport</strong><ul><li>Pathway and round</li><li>Program floors met</li><li>DTI estimate — in range or not</li><li>Verified rent months</li><li>Disputes: sent · open</li><li>Loan officer of record</li></ul></div><div className="no"><strong>Never inside</strong><ul><li>Your score</li><li>Your report or tradelines</li><li>SSN, DOB, address</li><li>Dispute letters</li></ul></div></div>;
...
const PAGES: Record<string, PageDef> = {
  "platform": { heroLayout: "stacked", kicker: "HOW IT WORKS", title: <>A decline is a stage, <em>not an ending.</em></>, lede: "A loan officer sends a not-ready borrower out — to a credit-repair partner or into ReadyIQ’s own plan. The file gets worked. When the borrower is approaching ready, the loan officer gets one signal and a summary they can act on: a stage and the blockers, never a score, never the report. Nobody is told they’re ready on a non-mortgage score.", accent: "teal", window: <FeedWindow />, windowLabel: "The three-score readiness view a loan officer sees", primary: ["See the consumer tools", DEMO + "?mode=consumer&cpage=result"], secondary: ["See the loan officer view", DEMO + "?mode=lender&lpage=link"],
    sections: [
      { kicker: "THE LOOP", title: <>The loan officer refers out. The partner works the file. <em>ReadyIQ brings the borrower back.</em></>, body: <p>Two doors, one loop. A loan officer declines a borrower and sends them to a credit-repair partner through ReadyIQ instead of losing them. The partner works the file. When the borrower crosses into the buffer band around the lender’s floor, ReadyIQ recommends a soft tri-merge; when they clear it, the partner presses <strong>Send to mortgage partner</strong> and the loan officer receives a summary with the borrower’s consent stamped on it.</p>, aside: <Loop />, layout: "center" },
      { kicker: "WHAT THE LOAN OFFICER SEES", title: <>Three scores and progress. <em>Never the private credit report.</em></>, body: <p>With the consumer’s permission, the loan officer sees Equifax, Experian and TransUnion scores together, plus the current journey stage, next milestone and request to reconnect. Accounts, dispute reasons, evidence and letters stay private.</p>, bullets: ["All three bureau scores, never one alone", "Current stage and next milestone", "The original loan officer, branch and source stay attached"], aside: <Frame label="Loan officer three-score pipeline"><FeedWindow /></Frame>, dark: true, layout: "rev" },
      { kicker: "WORKS WITH YOUR CURRENT TOOLS", title: <>No extra system <em>to manage.</em></>, body: <p>Notes, tasks and pipeline stay in your current mortgage system. ReadyIQ sends the three-bureau score summary and approved progress your team needs, so loan officers get one link and one clear feed instead of another daily queue.</p>, aside: <div className="stat-grid four"><Stat n="1" l="consumer link" /><Stat n="1" l="clear next action" /><Stat n="0" l="full reports shown" /><Stat n="3" l="scores in every score view" /></div>, layout: "band" },
      { kicker: "WHAT IT IS NOT", title: <>Not a credit repair company. Not a CRM. <em>Not a score simulator.</em></>, body: <p>The credit-repair firm is the credit repair organization; their CRM stays their system of record. ReadyIQ is the rail between them and the loan officer: the stage, the buffer, the consent record, and the audit trail — priced per seat, never per referral.</p>, aside: <Compare cols={["ReadyIQ", "Credit repair company", "CRM", "Score simulator"]} rows={[["Works the borrower’s file", "—", "✓", "—", "—"], ["Hands a borrower to a loan officer, with consent", "✓ the rail", "by text, maybe", "—", "—"], ["Tells the LO a stage and the blockers", "✓ never a score", "—", "—", "simulates"], ["Bills the consumer", "never", "yes", "—", "sometimes"], ["Paid per referral", "never", "—", "—", "—"], ["Loan officer sees the full report", "never", "—", "—", "yes"]]} />, layout: "center" },
    ...
    faq: [["What does the consumer do in ReadyIQ?", "They can check information from three bureaus, follow a personalized plan, review possible errors, create dispute letters, add eligible rent or bills, and track progress."], ["What does the loan officer see?", "With permission: Equifax, Experian and TransUnion scores together, approved progress, the current stage, the next milestone and a request to reconnect. The full credit report and dispute work stay private."], ["Is ReadyIQ a mortgage application?", "No. It helps the consumer work toward a future mortgage conversation. The consumer can apply whenever they choose, and the lender makes every lending decision."]] },

  "loan-officers": { kicker: "FOR LOAN OFFICERS", title: <>Who’s not ready. Who’s progressing. <em>Who’s ready to review.</em></>, lede: "Your declines don’t disappear — they get a stage. Send a not-ready borrower to a credit-repair partner through ReadyIQ, or into ReadyIQ’s own plan. Either way you see one feed with one blocker per person, and you get one signal when it’s time: ready to review.", accent: "teal", window: <LinkWindow />, windowLabel: "Your personal ReadyIQ link", primary: ["Get your link in 60 seconds", DEMO + "?mode=lender&lpage=start"], secondary: ["See the three-score pipeline", DEMO + "?mode=lender&lpage=borrowers"],
    sections: [
      { kicker: "60 SECONDS", title: <>Email and NMLS. <em>That’s the setup.</em></>, body: <p>We fill the rest from your NMLS ID and pull your company’s brand from its website. You get a link, a QR and a text you can send from your phone. No install, no training, no new queue.</p>, aside: <Steps items={[["Email + NMLS", "We fill the rest from your NMLS ID and pull your company’s brand from its website.", "about 20 seconds"], ["Your link + QR", "Text it, print it, put it in your signature. Attributed to you, branch and source.", "about 10 seconds"], ["Status, not work", "Consumers enroll themselves. You see pathway, round, milestones and review requests — in your feed or your CRM.", "from then on"]]} />, layout: "rev" },
      { kicker: "PRECISION, NOT VOLUME", title: <>The number that <em>keeps you.</em></>, body: <p>A tri-merge pull costs real money. So ReadyIQ measures itself on the only thing that matters to you: of the borrowers we said were ready to review, what share qualified on the formal pull. That number sits at the top of your feed, and the buffer tunes itself against it.</p>, aside: <div className="stat-grid four"><Stat n="71%" l="ready-to-review precision · illustrative" /><Stat n="+20" l="points above your floor before we say ready" /><Stat n="1" l="blocker per person, in words" /><Stat n="0" l="scores in the feed" /></div>, dark: true, layout: "band" },
      { kicker: "THE RECOMMENDED RE-PULL", title: <>Real mortgage scores — <em>soft, and only when it’s worth it.</em></>, body: <p>At readiness, your credit vendor can run a soft tri-merge with mortgage-model FICO® (2 · 4 · 5). The consumer sees the real numbers, you see “floors met,” and nobody’s credit takes a hard inquiry before you talk.</p>, aside: <Frame label="Review packet · soft tri-merge"><TriWindow /></Frame> },
    ],
    faq: [["What do I actually see?", "With consumer permission: Equifax, Experian and TransUnion scores together, pathway, round, next milestone, review requests and Protect Mode status. Never account details or the full report."], ["Where do my notes go?", "Your CRM. ReadyIQ sends status events through Zapier or a connector; it never asks you to work in a second system."], ["Can partners use it?", "Yes — realtors and property managers get their own links and QR; they see coarse status only."]] },

  "consumers": { heroLayout: "rev", kicker: "FOR BORROWERS", title: <>You’re not a decline. <em>You’re at a step.</em></>, lede: "ReadyIQ shows you where you are — Step 2 of 7 — and the one thing to do next. Your loan officer sees your stage, never your report. If a credit-repair company is working with you, they hand you back only with your say-so. You can apply anywhere, any time.", accent: "mint", window: <ScoreWindow />, windowLabel: "Your private ReadyIQ home", primary: ["See the consumer portal", DEMO + "?mode=consumer&cpage=result"], secondary: ["Have a link from your lender? Start here", DEMO + "?c=summit-jlee"],
    sections: [
      ...
      { kicker: "ON EVERY SCREEN", title: <>“You can apply for a mortgage at any time — <em>this is not required.</em>”</>, body: <p>The Reg B line is not fine print here. It is on the front door, the consents, the overview and the packet — because a readiness platform that pressures people to apply isn’t one.</p>, layout: "quote" },
    ...
    faq: [["Will this hurt my credit?", "No. The readiness check is a soft pull. A hard pull only happens when you request a review and say yes, when you talk to your loan officer."], ["Who sees what?", "With your permission, your loan officer sees your Equifax, Experian and TransUnion scores together plus pathway, round and milestones—not account details or the full report. Partners see only coarse status."], ["¿Está en español?", "Sí — el portal, la puerta de entrada y el registro cambian a español con un toque."]] },

  "credit-repair": { kicker: "FOR CREDIT REPAIR FIRMS", title: <>Win the LO relationships <em>you’re already working for.</em></>, lede: "Loan officers send you not-ready borrowers. You work the file the way you already do. When a client is approaching ready, one button sends a clean readiness summary back to the loan officer — no score, no report, no vague “making progress”.", accent: "lime", window: <BoardWindow />, windowLabel: "A case, the way the loan officer will see it", primary: ["See the credit-repair view", DEMO + "?mode=partner"], secondary: ["See what the loan officer receives", DEMO + "?mode=lender&lpage=borrowers"],
    sections: [
      { kicker: "WHY LOS STOP REFERRING", title: <>Vague updates, and no moment <em>to re-engage.</em></>, body: <p>“Making progress” is not something a loan officer can act on. ReadyIQ gives them the two things they never had: a stage they trust, and the exact moment a soft pull is worth their money. Give an LO that and they keep sending you files.</p>, layout: "center" },
      { kicker: "SEND TO MORTGAGE PARTNER", title: <>Exactly what travels — <em>and what never does.</em></>, body: <p>The summary carries the stage, the lender floors met, whether debt-to-income is in range, verified rent months, and dispute counts — open, resolved and withdrawn, kept separate so “we dropped it” never reads as “the bureau agreed”. It never carries a score, the report, balances, income or an SSN. Your client consents to that specific share, and the consent is stamped on the referral.</p>, aside: <InOut />, layout: "rev" },
      { kicker: "YOUR CASES", title: <>The same three groups <em>the loan officer sees.</em></>, body: <p>Not ready · Progressing · Ready to review. One vocabulary on both sides, so nobody argues about whether a client is “close”. Your CRM stays your system of record — ReadyIQ is the handoff, not another place to work.</p>, dark: true, layout: "center" },
      { kicker: "WHAT READYIQ IS NOT", title: <>You are the credit repair organization. <em>We are the rail.</em></>, body: <p>ReadyIQ never bills your client. It is priced per seat, never per referral, and nothing of value moves between you and a loan officer in either direction. You can send a client to more than one loan officer, and you should.</p>, bullets: ["Flat per-seat pricing", "Never a referral fee, in either direction", "Your client consents to each share and can withdraw it", "Multiple loan officers per client — never one by default"], layout: "center" },
    ],
    faq: [["Does this replace Credit Repair Cloud or DisputeFox?", "No. Keep working files where you work them. ReadyIQ connects through Zapier today and natively as vendors document their APIs; the handoff to the loan officer is what ReadyIQ adds."], ["What if the loan officer’s pull comes back short?", "They record it, and it counts against ReadyIQ’s precision number — not against you. That number is what tunes the buffer so early sends stop happening."]] },
  ...
  "products/check": { kicker: "PRODUCT · CREDIT CHECK", title: <>See all three bureaus. <em>Know where you stand.</em></>, lede: "With your permission, ReadyIQ shows FICO® scores and credit information from Equifax, Experian, and TransUnion through MyScoreIQ. It is a soft credit check, so viewing it does not hurt your score. We clearly explain that mortgage lenders may use different score versions.", accent: "mint", window: <ScoreWindow />, windowLabel: "Your three-bureau credit view", primary: ["See it in the portal", DEMO + "?mode=consumer&cpage=result"],
    sections: [{ kicker: "PATHWAYS", title: <>Ready now · near ready · build · thin · <em>dispute.</em></>, body: <p>The check assigns a pathway in plain words and a round count (“round 2 of ~5”), not a readiness percentage. Program floors and an eligibility clock (Chapter 7, foreclosure, short sale) set honest expectations.</p>, aside: <div className="stat-grid four"><Stat n="3" l="bureaus, every time" /><Stat n="0" l="hard inquiries" /><Stat n="5" l="pathways" /><Stat n="1" l="next action" /></div>, layout: "band" }, { kicker: "WHY IT MOVED", title: <>Every point, <em>tied to a cause.</em></>, body: <p>+14 utilization down, +6 lates aging, −6 a new inquiry. Consumers stop guessing and loan officers stop explaining.</p>, dark: true , aside: <Steps items={[["+14", "Utilization down — Summit Visa paid to $870.", "since last check"], ["+6", "Late payments aging — now 14 months old.", "since last check"], ["−6", "A new inquiry — Honda Financial.", "since last check"]]} />, layout: "rev" }, { kicker: "LABELED FOR WHAT IT IS", title: <>“FICO® — not the version lenders pull. <em>A guide, not a preapproval.</em>”</>, body: <p>That caption sits under every score in the product. When you are close, a soft tri-merge through your lender’s vendor can show the mortgage-model numbers — no hard inquiry.</p>, layout: "quote" }],
  ...
  "products/dispute-hub": { kicker: "PRODUCT · DISPUTE HUB", title: <>See what looks wrong. <em>Fix it step by step.</em></>, lede: "ReadyIQ organizes items from Equifax, Experian, and TransUnion in one place. You review one item at a time, explain what you believe is wrong, create your letter, and track the response. Nothing is sent unless you approve it.", accent: "violet", window: <BoardWindow />, windowLabel: "Items that may need your review", primary: ["Open the Dispute Hub", DEMO + "?mode=consumer&cpage=disputes"],
    sections: [{ kicker: "ONE ITEM AT A TIME", title: <>Is this information <em>right?</em></>, body: <p>Each item shows the facts, why CreditBuilderIQ flagged it, and why it matters for a mortgage — then one question. Say no and the letter writes itself; you read it and approve. Nothing is sent without you.</p>, bullets: ["Sequenced for a mortgage: collections that block programs first", "Letters only to the bureaus that actually report the item", "30-day clock kept for you; responses logged"] , aside: <Steps row items={[["Review", "Facts, the flag, why it matters — then one question.", "you decide"], ["Reason", "Five plain reasons. Bureaus pre-selected to the ones that report it.", "you choose"], ["Letter", "Written from your answers. Read it, edit it, approve it.", "you approve"], ["Track", "30 days (up to 45). Dates and outcomes kept for you.", "we keep the clock"]]} />, layout: "center" }, { kicker: "HONESTY", title: <>No promised <em>deletions.</em></>, body: <p>ReadyIQ does not claim accurate information can be removed or guarantee any result. Consumers dispute only what they believe is wrong — and the guideline notes explain when paying a collection does and doesn’t change the picture.</p>, dark: true }],
  ...
  "products/build-report": { kicker: "PRODUCT · RENT AND BILLS", title: <>Make the payments you already make <em>count.</em></>, lede: "CreditBuilderIQ can help verify eligible rent, electric, mobile, and other recurring payments and add them to your credit history. You choose what to connect and what to report. Results vary, and a score increase is never promised.", accent: "lime", window: <BuildWindow />, windowLabel: "Eligible rent history found", primary: ["See rent and bill reporting", DEMO + "?mode=consumer&cpage=reporting"],
    sections: [{ kicker: "RENT", title: <>24 months, <em>verified.</em></>, body: <p>CreditBuilderIQ matches the landlord, verifies the history and enrolls what the consumer chooses. Property-manager partners can start this a year before anyone talks to a lender.</p> , aside: <Steps row items={[["Match", "Landlord or property manager found from the payment trail.", "Scottsdale Palms · $1,840"], ["Verify", "24 on-time months confirmed; only eligible history is used.", "24 / 24"], ["Report", "Enrolled to all three bureaus. Fannie Mae’s DU counts it.", "Equifax · Experian · TransUnion"]]} />, layout: "center" }, { kicker: "EVERYDAY BILLS", title: <>Electric, mobile, <em>recurring.</em></>, body: <p>Eligible payments are matched, confirmed and added — no new traditional debt opened. Guidance on secured cards and credit-builder loans stays honest: check with your loan officer before opening or closing anything.</p>, dark: true }],
  ...
  "products/protect-mode": { kicker: "PRODUCT · PROTECT MODE", title: <>Protect your progress <em>until closing day.</em></>, lede: "Once your loan begins, Protect Mode pauses new dispute suggestions and reminds you to call your loan officer before opening an account, closing a card, moving money, or making a large payoff. ReadyIQ watches for important changes and shares only a simple alert with your loan officer.", accent: "teal", window: <ProtectWindow />, windowLabel: "Your closing protection checklist", primary: ["See Protect Mode", DEMO + "?mode=consumer&cpage=guardian"],
    sections: [{ kicker: "WHAT CHANGES", title: <>Nothing — <em>on purpose.</em></>, body: <p>No new accounts, no big payoffs, no surprise inquiries. MyScoreIQ monitoring flags anything new the same day; the loan officer receives a status, not a report. Closing-fund wire reminders included.</p>, bullets: ["Disputes paused automatically", "Daily report watch via MyScoreIQ", "LOS milestone (Encompass) can switch it on"] , aside: <Steps row items={[["Day one", "An LOS milestone (or the loan officer) switches Protect Mode on.", "Encompass · manual"], ["Every day", "MyScoreIQ watches the report; new inquiries and accounts are flagged the same day — to the consumer, and to the LO as status.", "daily"], ["Before closing", "Answer LOE requests within 24 hours, keep balances put, call the title company before wiring.", "the handful that blow up files"]]} />, layout: "center" }],
  ...
  "products/ask": { kicker: "PRODUCT · ASK READYIQ", title: <>Mortgage guidance <em>in plain English.</em></>, lede: "Ask ReadyIQ explains common FHA, Fannie Mae, and Freddie Mac guidelines in everyday words. It can help you understand collections, debt-to-income, rent history, and what to avoid before closing. It can also help draft a letter of explanation. Your lender still makes every lending decision.", accent: "mint", window: <AskWindow />, windowLabel: "Ask a mortgage-readiness question", primary: ["Try it in the portal", DEMO + "?mode=consumer&cpage=plan"],
    sections: [{ kicker: "GUIDELINE-AWARE", title: <>Why each step matters to an <em>underwriter.</em></>, body: <p>Every plan step carries a note: FHA’s $2,000 non-medical collections rule, pricing tiers for utilization, DU’s 12-month rent history, the credit refresh before closing. Guidance, not a promise — programs and overlays vary, and the loan officer has the final word.</p> , aside: <Steps row items={[["Ask", "“Do I have to pay the Midland collection before I apply?”", "the consumer, in plain words"], ["Guideline", "FHA: medical excluded; non-medical over $2,000 in aggregate → payment plan or 5% in DTI.", "public agency rules, summarized"], ["Draft", "A letter of explanation in the consumer’s own voice — edited, then shared only if they choose.", "never a promise"]]} />, layout: "center" }],
  ...
  "integrations": { kicker: "INTEGRATIONS", title: <>ReadyIQ progress, <em>inside the tools you already use.</em></>, lede: "Today: CSV import, Zapier, signed webhooks and a status API — enough to reach every CRM on this page. Shape and Total Expert connectors are available; Encompass, Blend and LenderHomePage come by partner agreement. On the credit-repair side, DisputeChat is native and Credit Repair Cloud and DisputeFox connect through Zapier.", accent: "teal", window: <EventsWindow />, windowLabel: "Progress updates moving between systems", primary: ["Open the integration hub", DEMO + "?mode=integrations"],
  ...
  "trust": { heroLayout: "stacked", kicker: "TRUST & PRIVACY", title: <>Clear permission. <em>Protected information.</em></>, lede: "Consumers approve every credit check and every share. With permission, the lender receives all three bureau scores and simple progress updates—not the full credit report. ReadyIQ does not promise score increases, deletions, or approvals. Consumers can stop sharing whenever they choose.", accent: "teal", window: <FeedWindow />, windowLabel: "Exactly what a loan officer can see",
    sections: [
      { kicker: "YOUR PERMISSION", title: <>Three clear choices, <em>before anything happens.</em></>, body: <p>Before checking credit, ReadyIQ asks the consumer to approve three things separately: view my credit for my own review, share my three bureau scores and progress with my loan officer, and send me text updates. A separate permission is required before a lender checks mortgage credit.</p>, aside: <Frame label="The choices exactly as the consumer sees them"><div className="consent-demo"><ConsentBlock onChange={() => {}} /></div></Frame>, layout: "rev" },
      { kicker: "WHO SEES WHAT", title: <>Each person sees only <em>what their role needs.</em></>, body: <p>The table is the product: it is enforced in the lender pipeline, progress updates, the Passport and every screen.</p>, aside: <Compare hi={-1} cols={["Consumer", "Loan officer", "Partner", "Passport recipient"]} rows={[["Three-bureau scores", "✓", "✓ with permission", "—", "—"], ["Report · account details", "✓", "—", "—", "—"], ["Pathway · round · milestones", "✓", "✓", "coarse", "✓"], ["Review request + packet", "✓", "✓ with consent", "—", "—"], ["Protect Mode status", "✓", "✓", "—", "✓"], ["SSN · DOB · address", "✓", "—", "—", "—"]]} />, layout: "center" },
      { kicker: "WHAT STAYS PRIVATE", title: <>Your full report, accounts, and SSN <em>stay private.</em></>, body: <p>The assigned loan officer receives the consumer-authorized Equifax, Experian and TransUnion score summary plus progress. Account-level report details, dispute evidence and the full report stay private. Partners and the ReadyIQ Passport never receive scores. A consumer can stop sharing at any time.</p>, dark: true, bullets: ["Every score surface shows all three bureaus together", "No promised deletions, score increases, approvals, or timelines", "MyScoreIQ and CreditBuilderIQ are named wherever their tools appear"], aside: <div className="stat-grid four"><Stat n="3" l="bureau scores shown together" /><Stat n="0" l="full reports shared" /><Stat n="3" l="clear choices before the check" /><Stat n="1" l="tap to stop sharing" /></div>, layout: "band" },
    ...
    faq: [["Is ReadyIQ a credit repair organization?", "No. ReadyIQ is software sold to loan officers, mortgage companies and credit-repair firms. It never charges the borrower, never promises a result, and never takes a fee for a referral. Where a credit-repair firm works a file through ReadyIQ, that firm is the credit repair organization under CROA and its state equivalents."], ["How does ReadyIQ stay on the right side of RESPA?", "Flat per-seat pricing — never per referral, never per funded loan. A credit-repair firm can send a borrower to more than one loan officer, and nothing of value moves between the two sides. The borrower consents to each share, and the consent is stamped on the referral and kept."], ["How is the texting compliant?", "Opt-in with express consent language, message rates disclosure and STOP handling on the consent itself."]] },
          ...
          {!sent ? <>
            ...
            <div className="form-panel" style={{ padding: 0 }}>
              <div className="form-grid">
                ...
                <label>Your role<select defaultValue="lo"><option value="lo">Loan officer</option><option>Credit repair owner / operator</option><option>Branch manager</option><option>Marketing or growth</option><option>Privacy or compliance</option><option>Technology</option></select></label>
              </div>
              <label className="full-field">What matters most to you?<select defaultValue="all"><option value="all">See the whole experience</option><option>My link and progress updates</option><option>The consumer experience</option><option>Sending clients to loan officers</option><option>Connections to my current tools</option><option>Privacy and permissions</option></select></label>
...
function HomePage() {
  ...
  return <div className="site-home"><ReadyIQWebsite openOrganization={to(DEMO + "?mode=lender&lpage=link")} openStart={to(DEMO + "?mode=lender&lpage=start")} openConsumer={(page = "welcome") => { location.href = page === "welcome" ? DEMO + "?mode=consumer" : DEMO + "?mode=consumer&cpage=" + page; }} openIntegrations={to("integrations/")} />
    <section className="site-section"><div className="site-section-inner one"><span className="section-kicker">CHOOSE YOUR VIEW</span><h2>See the part that matters <em>to you.</em></h2><div className="mini-cards four">{([["How it works", "A decline becomes a stage. A partner works the file. One signal comes back.", "platform/"], ["For loan officers", "Who’s not ready, who’s progressing, who’s ready to review — one blocker per person.", "loan-officers/"], ["For credit repair firms", "Send a client back to their loan officer with consent, in one screen.", "credit-repair/"], ["Trust and compliance", "Stages not scores, flat pricing, and why that keeps everyone on the right side of RESPA.", "trust/"]] as [string, string, string][]).map(([t, d, h]) => <a key={t} href={h} className="mini-card"><strong>{t}</strong><p>{d}</p><b>Explore →</b></a>)}</div></div></section>
    <TestimonialSection />
    <CtaBand title={<>Turn declines into a <em>managed pipeline.</em></>} sub="Every not-ready borrower gets a stage, a plan, and a way back to you. Credit-repair partners work the file. You get one signal when it’s time." />
  </div>;
}
