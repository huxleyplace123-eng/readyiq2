...
const PAGES: Record<string, PageDef> = {
  "platform": { heroLayout: "stacked", kicker: "PLATFORM · WHAT READYIQ DOES", title: <>Credit tools for consumers. <em>Progress for loan officers.</em></>, lede: "Consumers get a private place to check all three bureaus, review possible errors, create dispute letters, add eligible rent and bills, and follow a personalized plan. Loan officers see approved progress—not the private credit report—and reconnect when the consumer is ready for a mortgage conversation.", accent: "teal", window: <FeedWindow />, windowLabel: "The progress updates a loan officer sees", primary: ["See the consumer tools", DEMO + "?mode=consumer&cpage=result"], secondary: ["See the loan officer view", DEMO + "?mode=lender&lpage=link"],
    sections: [
      ...
      { kicker: "WHAT IT IS NOT", title: <>Not a CRM. Not a score simulator. <em>Not a counseling program.</em></>, body: <p>Each of those does one thing well. ReadyIQ does the thing none of them do: it gives the consumer a private place to check, fix and build, and sends approved progress updates to the loan officer’s tools.</p>, aside: <Compare cols={["ReadyIQ", "CRM", "Score simulator", "Counseling network"]} rows={[["A private workspace for the consumer", "✓ start to review", "—", "—", "partly"], ["Dispute + rent-reporting tools", "✓ CreditBuilderIQ", "—", "—", "—"], ["All three bureaus, monitored", "✓ MyScoreIQ", "—", "—", "—"], ["Progress updates into your tools", "✓ events + object", "is the CRM", "—", "—"], ["Promises a score", "never", "—", "simulates", "—"], ["Loan officer sees the report", "never", "—", "yes", "varies"]]} />, layout: "center" },
  ...
  "trust": { heroLayout: "stacked", kicker: "TRUST & PRIVACY", title: <>Clear permission. <em>Protected information.</em></>, lede: "Consumers approve every credit check and every share. The lender receives simple progress updates—not the credit report. ReadyIQ does not promise score increases, deletions, or approvals. Consumers can stop sharing whenever they choose.", accent: "teal", window: <FeedWindow />, windowLabel: "Exactly what a loan officer can see",
    sections: [
      ...
      { kicker: "WHO SEES WHAT", title: <>Each person sees only <em>what their role needs.</em></>, body: <p>The table is the product: it is enforced in the progress updates, the Passport and every screen.</p>, aside: <Compare hi={-1} cols={["Consumer", "Loan officer", "Partner", "Passport recipient"]} rows={[["Three-bureau scores", "✓", "—", "—", "—"], ["Report · account details", "✓", "—", "—", "—"], ["Pathway · round · milestones", "✓", "✓", "coarse", "✓"], ["Review request + packet", "✓", "✓ with consent", "—", "—"], ["Protect Mode status", "✓", "✓", "—", "✓"], ["SSN · DOB · address", "✓", "—", "—", "—"]]} />, layout: "center" },
