...
const Loop = () => <div className="loop-cards">{[["01", "Capture", "Invite from your website, CRM, POS, a partner, a building — or the not-ready pile you already paid for.", "paper"], ["02", "Check", "A consumer-permissioned soft check on three bureaus creates the starting point. MyScoreIQ.", "mint"], ["03", "Act", "Disputes by bureau, rent and bills reported, a plan in priority order. CreditBuilderIQ.", "lime"], ["04", "Return", "Program floors met, DTI in range → the consumer is asked, not you. Status returns to the original LO.", "dark"]].map(([n, t, d, tone]) => <article key={n} className={tone}><span>{n}</span><strong>{t}</strong><p>{d}</p></article>)}</div>;

/* ---------- pages ---------- */
const PAGES: Record<string, PageDef> = {
  "platform": { heroLayout: "stacked", kicker: "PLATFORM · HOW IT WORKS", title: <>One clear path from “not yet” to <em>ready to talk.</em></>, lede: "ReadyIQ gives each consumer a private place to check their credit, follow a simple plan, fix information they believe is wrong, and build positive history. Your team receives progress updates in the systems it already uses—without seeing the consumer’s credit report.", accent: "teal", window: <FeedWindow />, windowLabel: "The progress updates your team sees", primary: ["Try the lender portal", DEMO + "?mode=lender&lpage=link"], secondary: ["Book a demo", "book-a-demo/"],
    sections: [
      { kicker: "ONE CONNECTED LOOP", title: <>From credit-challenged lead to <em>lender review.</em></>, body: <p>Four steps, one owner. ReadyIQ sits between your acquisition channels, the consumer’s credit tools and your mortgage systems — keeping ownership, consent and next steps clear at every step.</p>, aside: <Loop />, layout: "center" },
      { kicker: "THE CONTRACT", title: <>One status object. <em>Never a report.</em></>, body: <p>What flows into your CRM is a status object — pathway, round, next milestone, flags, attribution — and a handful of webhook events. Never a score, never a report, never tradelines. That is the whole integration surface, and it is why compliance says yes.</p>, bullets: ["readiness.trigger · review.requested · protect_mode.activated · progress.milestone_reached", "Attribution to the loan officer, branch, partner and source on every event", "Consumers can pause sharing — you get a paused status, nothing else"], aside: <Frame label="Integration hub · events"><EventsWindow /></Frame>, dark: true, layout: "rev" },
      { kicker: "NOT A CRM, ON PURPOSE", title: <>The CRM stays the <em>system of record.</em></>, body: <p>Notes, tasks, pipeline and campaigns stay in Total Expert, Shape or Salesforce. ReadyIQ sends status through Zapier or a native connector and asks for nothing back. Loan officers get one link and one read-only feed — no new queue to babysit.</p>, aside: <div className="stat-grid four"><Stat n="1" l="next action at a time" /><Stat n="1:1" l="loan officer attribution" /><Stat n="0" l="promised deletions, points or approvals" /><Stat n="3" l="bureaus, always" /></div>, layout: "band" },
      { kicker: "THE RAILS NOBODY ELSE HAS", title: <>Four rails, <em>one roof.</em></>, body: <p>Rent-reporting rails (CreditBuilderIQ / Resident-Link), monitoring and identity protection (MyScoreIQ), a dispute engine, and a lender network. Counseling networks have one. Consumer apps have two. Score simulators have none. The moves that matter — the Passport, renters starting early, the soft tri-merge last mile — exist because all four are under one roof.</p>, aside: <Cards items={[["Readiness Passport", "A verified, portable status the consumer owns.", "products/passport/"], ["Renters start early", "Property-manager links start rent history 12–24 months before a loan officer.", "partners/"], ["The soft tri-merge last mile", "Real mortgage-model scores at readiness, no hard pull.", "loan-officers/"]]} />, layout: "grid" },
    ],
    faq: [["Is ReadyIQ a CRM?", "No. It owns the consumer journey and sends status. Notes, tasks and pipeline stay where they are."], ["Do loan officers ever see the credit report?", "No. They see pathway, round, milestones and review requests. A review packet is status plus consent for the hard pull when they talk."], ["Are the scores mortgage scores?", "MyScoreIQ shows FICO® on three bureaus, labeled as not the mortgage-industry version. At readiness, a soft tri-merge via the lender’s vendor shows the real mortgage-model numbers without a hard inquiry."]] },
