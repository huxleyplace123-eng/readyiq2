...
const Compare = ({ cols, rows, hi = 0 }: { cols: string[]; rows: string[][]; hi?: number }) => <div className="compare"><table><thead><tr><th /> {cols.map((c, i) => <th key={c} className={i === hi ? "hi" : ""}>{c}</th>)}</tr></thead><tbody>{rows.map((r) => <tr key={r[0]}><th>{r[0]}</th>{r.slice(1).map((v, i) => <td key={i} className={`${i === hi ? "hi" : ""} ${v.startsWith("✓") ? "yes" : v === "—" || v.startsWith("never") ? "no" : ""}`}>{v}</td>)}</tr>)}</tbody></table></div>;
