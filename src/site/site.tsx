...
export function SiteNav({ route }: { route: Route }) {
  ...
  const who: [string, string][] = [["loan-officers/", "Loan officers — three scores + progress"], ["consumers/", "Consumers — check, build, dispute"], ["partners/", "Partners — realtors and buildings"]];
...
export function HonestyStrip() {
  return <div className="honesty"><span>✓ Three scores together, never the full report</span><span>✓ No promised deletions, points or approvals</span><span>✓ Soft pull only — apply at any time, never required</span><span>✓ MyScoreIQ + CreditBuilderIQ, named on every screen</span></div>;
}
...
const PAGES: Record<string, PageDef> = {
  "platform": { heroLayout: "stacked", kicker: "PLATFORM · WHAT READYIQ DOES", title: <>Credit tools for consumers. <em>Progress for loan officers.</em></>, lede: "Consumers get a private place to check all three bureaus, review possible errors, create dispute letters, add eligible rent and bills, and follow a personalized plan. With permission, loan officers see all three bureau scores and approved progress—not the full credit report—and reconnect when the consumer is ready for a mortgage conversation.", accent: "teal", window: <FeedWindow />, windowLabel: "The three-score readiness view a loan officer sees", primary: ["See the consumer tools", DEMO + "?mode=consumer&cpage=result"], secondary: ["See the loan officer view", DEMO + "?mode=lender&lpage=link"],
    sections: [
      ...
      { kicker: "WHAT IT IS NOT", title: <>Not a CRM. Not a score simulator. <em>Not a counseling program.</em></>, body: <p>Each of those does one thing well. ReadyIQ does the thing none of them do: it gives the consumer a private place to check, fix and build, and—with permission—sends all three scores and approved progress to the loan officer’s tools.</p>, aside: <Compare cols={["ReadyIQ", "CRM", "Score simulator", "Counseling network"]} rows={[["A private workspace for the consumer", "✓ start to review", "—", "—", "partly"], ["Dispute + rent-reporting tools", "✓ CreditBuilderIQ", "—", "—", "—"], ["All three bureaus, monitored", "✓ MyScoreIQ", "—", "—", "—"], ["Three-score progress into your tools", "✓ with permission", "—", "—", "—"], ["Promises a score", "never", "—", "simulates", "—"], ["Loan officer sees the full report", "never", "—", "yes", "varies"]]} />, layout: "center" },
  ...
  "loan-officers": { kicker: "FOR LOAN OFFICERS", title: <>One link. <em>Zero busywork.</em></>, lede: "In about a minute, you get a personal link you can text, print, or email. Every consumer stays connected to you. With permission, you see all three bureau scores and simple progress updates—not their full credit report—and ReadyIQ brings them back when they are ready for another conversation.", accent: "teal", window: <LinkWindow />, windowLabel: "Your personal ReadyIQ link", primary: ["Get your link in 60 seconds", DEMO + "?mode=lender&lpage=start"], secondary: ["See the three-score pipeline", DEMO + "?mode=lender&lpage=borrowers"],
    sections: [
  ...
  "consumers": { heroLayout: "rev", kicker: "FOR CONSUMERS", title: <>Understand your credit. Take the <em>next right step.</em></>, lede: "See information from all three credit bureaus, follow a plan made for you, review anything that looks wrong, and add eligible rent or bill payments. With your permission, your loan officer sees all three scores and your progress—not your private credit report. You can apply for a mortgage whenever you choose.", accent: "mint", window: <ScoreWindow />, windowLabel: "Your private ReadyIQ home", primary: ["See the consumer portal", DEMO + "?mode=consumer&cpage=result"], secondary: ["Have a link from your lender? Start here", DEMO + "?c=summit-jlee"],
    sections: [
