...
/* Guardian — on when a loan file is active */
export function GuardianPage({ on, setOn }: { on: boolean; setOn: (v: boolean) => void }) {
  return <div className="dashboard-page progress-page">
    <div className="welcome-row"><div><span className="section-kicker">GUARDIAN</span><h2>{on ? <>Your loan file is active. <em>Guardian is on.</em></> : <>Guardian turns on when your loan file is <em>active.</em></>}</h2><p>{on ? "Nothing changes on your report without Jordan knowing — and nothing should change on purpose without asking first." : "From application to closing, Guardian pauses dispute suggestions, watches your report daily and reminds you to ask Jordan before you open, close or pay off anything."}</p></div><button className={on ? "outline-button" : "primary-dark"} onClick={() => setOn(!on)}>{on ? "Turn Guardian off (demo)" : "Simulate an active loan file (demo)"}</button></div>
