...
export function EnrollSections({ onComplete, autoScroll }: { onComplete: () => void; autoScroll?: boolean }) {
  ...
  return <section className="door-flow" id="start" ref={ref}>
    <div className="door-head"><span className="section-kicker">{es ? "TRES TARJETAS · UNOS CINCO MINUTOS · NO AFECTA TU PUNTAJE" : "THREE SHORT CARDS · ABOUT FIVE MINUTES · NO EFFECT ON YOUR SCORE"}</span><h2>{es ? <>Empieza tu <em>revisión.</em></> : <>Start your <em>readiness check.</em></>}</h2><p>{es ? "Llena las tres tarjetas. No se consulta nada hasta que tú lo digas en la última — y Summit ve tu estatus, nunca tu reporte." : "Fill in the three cards below. Nothing is pulled until you say so on the last one — and Summit sees status, never your report."}</p></div>
    <div className="door-grid">
      ...
      <div className="door-cards">
        ...
        <article className="form-panel permission-panel door-card" id="s2">
          ...
          <div className="permission-list"><div><i>✓</i><p><strong>{es ? "Es una consulta suave" : "This is a soft credit check"}</strong><span>{es ? "Revisar tu preparación no baja tu puntaje." : "Checking your readiness will not lower your score."}</span></p></div><div><i>⌁</i><p><strong>{es ? "Tu información está protegida" : "Your information stays protected"}</strong><span>{es ? "Summit recibe hitos permitidos — no acceso a tu reporte." : "Summit receives permitted milestones — not unrestricted report access."}</span></p></div><div><i>≠</i><p><strong>{es ? "No es una preaprobación" : "This is not a mortgage preapproval"}</strong><span>{es ? "Tu prestamista usa un reporte hipotecario aparte cuando pides revisión." : "Your lender uses a separate mortgage report when you request review."}</span></p></div></div>
